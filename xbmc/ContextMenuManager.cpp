  ...
  const CFileItemPtr& fileItem, CContextButtons& list, const CContextMenuItem& root /* = CContextMenuItem::MAIN */) const
{
  ...
  auto menuItems = GetAddonMenus(*fileItem, root);
  for (const auto& kv : menuItems)
    list.emplace_back(kv.first, kv.second.GetLabel(*fileItem));
}
std::vector<std::pair<unsigned int, CContextMenuItem>> CContextMenuManager::GetAddonMenus(
...
ContextMenuView CContextMenuManager::GetItems(const CFileItem& fileItem,
    const CContextMenuItem& root /*= MAIN*/, bool includeAddons /*= true*/) const
{
  ...
  if (includeAddons)
  {
    for (const auto& kv : GetAddonMenus(fileItem, root))
